...
export const TrainerShowcase = () => {
  const { users } = useApp();
  const trainers = users.filter(u => u.role === 'trainer' && (u.isApproved === true || u.status === 'approved'));
    ...
    <section className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto border-t border-slate-200 dark:border-slate-800">
      ...
      {trainers.length === 0 ? (
        <div className="glass-card rounded-2xl p-10 text-center border border-slate-200 dark:border-slate-800">
          ...
          <h3 className="text-base font-bold text-slate-700 dark:text-slate-200">Faculty Directory</h3>
          <p className="text-xs text-slate-500 mt-1 max-w-md mx-auto">
            Faculty profiles are managed securely in the MoES administration portal. Log in as an administrator or trainer to manage credentials.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {trainers.map(trainer => (
            ...
            >
              <img
                src={trainer.avatar || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80"}
                alt={trainer.name}
              ...
              <div className="space-y-2 flex-1">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-xs text-moes-600 dark:text-sky-400 font-semibold">{trainer.designation || 'Faculty Instructor'}</p>
                    <p className="text-[11px] text-slate-500">{trainer.organization || 'Ministry of Earth Sciences'}</p>
                  </div>
                ...
                <p className="text-xs text-slate-600 dark:text-slate-400 line-clamp-2 leading-relaxed">
                  {trainer.bio || 'MoES scientific expert and capacity building program mentor.'}
                </p>

                <div className="pt-2">
                  <div className="flex flex-wrap gap-1.5">
                    {(trainer.specializations || ['Meteorology', 'Oceanography']).slice(0, 3).map((spec, i) => (
                      <span
                        key={i}
